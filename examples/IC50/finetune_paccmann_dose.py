...
parser.add_argument(
    'finetuning_model_name', type=str,
    help='Name of the model to be finetuned.'
)
parser.add_argument(
    'training_name', type=str,
    help='Name of the resulting finetuned model.'
)
# yapf: enable
...
def main(
    ...
    params_filepath, finetuning_model_name, training_name
):
    ...
    ft_model_dir = os.path.join(model_path, finetuning_model_name)
    ...
    # Assemble datasets
    train_dataset = DrugSensitivityDoseDataset(
    ...
    model_name = params.get('model_fn', 'paccmann_dose')
    model = MODEL_FACTORY[model_name](params).to(device)
    model._associate_language(smiles_language)

    if os.path.isfile(
        os.path.join(ft_model_dir, 'weights', f'best_mse_{model_name}.pt')
    ):
    ...
    for epoch in range(params['epochs']):
        ...
        for ind, (smiles, gep, dose, y) in enumerate(train_loader):
            y_hat, pred_dict = model(smiles, gep, dose)
...
if __name__ == '__main__':
    ...
    main(
        ...
        args.finetuning_model_name, args.training_name
    )
